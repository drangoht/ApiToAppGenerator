# Apivolt (formerly API-to-App Generator)

**🌐 Live Demo:** [https://Apivolt.thognard.net/](https://Apivolt.thognard.net/)

Apivolt is a powerful, AI-driven platform that instantly transforms raw OpenAPI Specifications into fully functional, stunning, and deployable Next.js applications.

By simply uploading a `swagger.json` or `openapi.yaml`, Apivolt's intelligent generation engine parses the endpoints and uses advanced Large Language Models to write a complete frontend architecture—including API clients, state management, and beautiful UI components—specifically tailored to interact with your target API.

![Apivolt Logo](./public/logo.png "Apivolt")
...
Apivolt is built to be modern, fast, and scalable. Here is a breakdown of the core technologies powering the platform:
...
- **[React 18](https://react.dev/):** Provides the component-driven architecture for both the Apivolt generator and the applications it creates.

### 2. Styling & UI Design System

- **[Tailwind CSS](https://tailwindcss.com/):** For rapid, utility-first styling and creating the custom Apivolt HSL gradient color palette.
...
1. **Parsing Phase:** When a user uploads an OpenAPI spec, Apivolt parses the YAML/JSON to extract available endpoints, methods, parameters, and descriptions.
...
5. **File Hydration:** Apivolt intercepts the LLM output, splits it into localized files within a secure `projects/{id}/generated` directory, and actively overwrites the `package.json` and `next.config.mjs` to inject failsafes and ensure the generated app doesn't crash the Node process.
...
Once inside your newly created project, upload a valid JSON or YAML **OpenAPI 3.0+ Specification**. Apivolt will parse all endpoints and display them in the **API Spec** tab.
...
![Apivolt API Spec Tab](./public/docs/project_api_tab.png)
...
![Apivolt Settings Tab](./public/docs/project_settings_tab.png)

### 4. Generate the App
Click the **Generate App** button in the project header. Apivolt compiles your spec, enrichments, and LLM settings into a comprehensive prompt and generates the full Next.js frontend.
...
![Apivolt Preview Tab](./public/docs/project_preview_tab.png)
...
![Apivolt Download Tab](./public/docs/project_download_tab.png)
...
Apivolt includes a production-ready `docker-compose` setup that packages the Next.js App Router and maintains persistent local volumes out of the box.
...
To deploy Apivolt to a remote server without pulling the full source code, you can use the pre-built Docker image and a minimal `docker-compose.yml` file.

1. **Connect to your Server** and create a deployment directory:
   ```bash
   mkdir -p ~/Apivolt/prisma
   mkdir -p ~/Apivolt/projects
   cd ~/Apivolt
   ```
...
4. **Create the `docker-compose.yml` File:**
   Create a `docker-compose.yml` inside the `Apivolt` directory:
   ```yaml
   services:
     Apivolt:
